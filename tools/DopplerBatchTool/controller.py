...
class Controller:
    ...
    def _clear_logs_and_state(self):
        ...
        for log_area in [self.ui.holo_cine_log_area, self.ui.hd_log_area, self.ui.ef_log_area]:
            log_area.config(state=tk.NORMAL)
        ...
        for button in [self.ui.export_hd_button, self.ui.export_holo_cine_button, 
                       self.ui.export_missing_hd_button, self.ui.export_ef_button, 
                       self.ui.export_missing_ef_button, self.ui.export_ef_results_button]:
    ...
    def start_search(self):
        ...
        if not input_file or not root_folder:
            self.ui.status_label.config(text="Please select both an input file and a root folder.")
            return
        ...
        self.holo_cine_files = logic.find_holo_cine_files(
            self.identifiers, root_folder,
            progress_callback=self._update_progress_bar,
            error_callback=lambda msg: self._log_message(self.ui.holo_cine_log_area, msg)
        )
    ...
    def _finalize_search_and_update_gui(self):
        ...
        if self.holo_cine_files: self.ui.export_holo_cine_button.config(state=tk.NORMAL)
        if self.latest_hd_folders: self.ui.export_hd_button.config(state=tk.NORMAL)
        if self.hd_batch_input: self.ui.export_missing_hd_button.config(state=tk.NORMAL)
        ...
        if self.ef_batch_input: self.ui.export_missing_ef_button.config(state=tk.NORMAL)
            ...
            try:
                with open(export_filepath, 'w') as f:
                    for path in data_list:
                        f.write(f"{path}\n")
                self.ui.status_label.config(text=f"Successfully exported {len(data_list)} paths.")
            except Exception as e:
    ...
    def export_hd_folders(self):
        self._export_list_to_file(self.latest_hd_folders, "Save Latest HD Folder Paths As")

    def export_hd_batch_input(self):
        self._export_list_to_file(self.hd_batch_input, "Save .holo/.cine files not processed by HD as")

    def export_ef_folders(self):
        self._export_list_to_file(self.latest_ef_folders, "Save Latest EF Folder Paths As")
    ...
    def export_ef_results(self):
        ...
        self.ui.progress_frame.pack_forget()
        if errors:
            self.ui.status_label.config(text=f"Export complete with {len(errors)} errors. Copied {copied_count} files.")
        else:
            self.ui.status_label.config(text=f"Successfully exported results. Copied {copied_count} files.")
            if not logic.open_folder(destination_root):
                 self._log_message(self.ui.ef_log_area, "[INFO] Could not open destination folder automatically.")