...
class AppUI:
    ...
    def _setup_export_frames(self, parent):
        export_found_frame = tk.LabelFrame(
            parent, text="Export Found Items", padx=5, pady=5
        )
        export_found_frame.pack(fill=tk.X, pady=5)
        for i in range(2):
            export_found_frame.columnconfigure(i, weight=1)

        self.export_holo_cine_button = tk.Button(
            export_found_frame, text="Export .holo/.cine paths", state=tk.DISABLED
        )
        self.export_holo_cine_button.grid(
            row=0, column=0, padx=(0, 5), pady=5, ipady=5, sticky="ew"
        )

        self.export_hd_button = tk.Button(
            export_found_frame, text="Export latest HD folder paths", state=tk.DISABLED
        )
        self.export_hd_button.grid(
            row=0, column=1, padx=(5, 0), pady=5, ipady=5, sticky="ew"
        )

        self.export_ef_button = tk.Button(
            export_found_frame, text="Export latest EF folder paths", state=tk.DISABLED
        )
        self.export_ef_button.grid(
            row=1, column=0, padx=(0, 5), pady=5, ipady=5, sticky="ew"
        )

        self.export_ef_results_button = tk.Button(
            export_found_frame, text="Export EF results", state=tk.DISABLED
        )
        self.export_ef_results_button.grid(
            row=1, column=1, padx=(5, 0), pady=5, ipady=5, sticky="ew"
        )

        export_missing_frame = tk.LabelFrame(
            parent, text="Export Batch Inputs", padx=5, pady=5
        )
        export_missing_frame.pack(fill=tk.X, pady=5)
        for i in range(2):
            export_missing_frame.columnconfigure(i, weight=1)

        self.export_missing_hd_button = tk.Button(
            export_missing_frame,
            text="Export HoloDoppler Batch Input",
            state=tk.DISABLED,
        )
        self.export_missing_hd_button.grid(
            row=0, column=0, padx=(0, 5), ipady=5, sticky="ew"
        )

        self.export_missing_ef_button = tk.Button(
            export_missing_frame, text="Export EyeFlow Batch Input", state=tk.DISABLED
        )
        self.export_missing_ef_button.grid(
            row=0, column=1, padx=(5, 0), ipady=5, sticky="ew"
        )
    ...
    def _create_log_tab(self, notebook, text):
        ...
        log_area = scrolledtext.ScrolledText(
            tab, wrap=tk.WORD, height=10, state=tk.DISABLED
        )
    ...
    def _setup_bottom_frame(self, parent):
        ...
        self.status_label = tk.Label(
            bottom_frame,
            text="Please select your input file and root folder.",
            relief="sunken",
            anchor="w",
            padx=5,
        )
