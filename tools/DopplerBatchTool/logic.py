...
def find_holo_cine_files(
    identifiers, root_folder, progress_callback=None, error_callback=None
):
    """
    ...
    for i, identifier in enumerate(identifiers):
        ...
        if not matching_top_dirs:
            if error_callback:
                error_callback(
                    f"[ERROR] No directory starting with '{prefix}' found for identifier '{identifier}'."
                )
            ...
            for dirpath, _, filenames in os.walk(target_dir_path):
                for filename in filenames:
                    if filename.startswith(identifier) and (
                        filename.endswith(".holo") or filename.endswith(".cine")
                    ):
        ...
        if files_found_for_id == 0:
            if error_callback:
                error_callback(
                    f"[ERROR] No '.holo' or '.cine' files for '{identifier}' found in its matching directory/directories."
                )
...
def find_latest_hd_folders(holo_cine_files, error_callback=None):
    ...
    for file_path in holo_cine_files:
        ...
        if candidate_folders:
            ...
            if error_callback:
                error_callback(
                    f"[WARNING] For '{source_filename}', no corresponding HD folder was found."
                )
...
def find_latest_ef_folders(hd_folder_paths, error_callback=None):
    ...
    for hd_path in hd_folder_paths:
        ...
        if candidate_folders:
            ...
            if error_callback:
                error_callback(
                    f"[WARNING] 'eyeflow' directory exists but no EF folders found in: {hd_path}"
                )
            missing_ef_folders.append(hd_path)
...
def copy_ef_results(
    ...
    for i, ef_folder in enumerate(ef_folders):
        ...
        matched_identifier = next(
            (iden for iden in identifiers if ef_folder_name.startswith(iden)), None
        )

        if not matched_identifier:
            error_msg = (
                f"Could not match EF folder '{ef_folder_name}' to any input identifier."
            )
            error_list.append(error_msg)
            if error_callback:
                error_callback(f"[EXPORT ERROR] {error_msg}")
            continue
