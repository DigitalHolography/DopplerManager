...
def render_export_section(filtered_ef_df: pd.DataFrame):
    ...
    if st.button("Prepare Export Package"):
        ...
        with st.spinner("Collecting file paths..."):
            for _, row in filtered_ef_df.iterrows():
                ...
                if (
                    export_h5s
                    and row.get("ef_h5_output")
                    and pd.notna(row["ef_h5_output"])
                ):
                        ...
                        for json_file in json_dir.glob("*.json"):
                            if json_file not in seen_paths:
                                arcname = os.path.join(
                                    base_folder, "json", json_file.name
                                )
                                files_to_zip.append(
                                    {"path": json_file, "arcname": arcname}
                                )
    ...
    st.markdown("---")
    st.subheader("Export EF Parameters")

    if st.button("Generate EF Parameters CSV"):
        all_records = []
        with st.spinner("Reading and processing JSON files..."):
            for _, row in filtered_ef_df.iterrows():
                ef_folder = Path(row["ef_folder"])
                json_path = ef_folder / "json" / "InputEyeFlowParams.json"
                ...
                if json_path.exists() and json_path.is_file():
                    try:
                        ...
                        # Add identifying information to the record
                        record = {
                            "holo_file": row["holo_file"],
                            "measure_tag": row["measure_tag"],
                            "hd_folder": row["hd_folder"],
                            "ef_folder": row["ef_folder"],
                        }
                        record.update(flattened_data)
                        all_records.append(record)

                    except Exception as e:
                        st.warning(f"Could not process file {json_path}: {e}")

        if not all_records:
            st.warning(
                "No 'InputEyeFlowParams.json' files found for the current selection."
            )
            return

        # Convert the list of dictionaries to a DataFrame
        csv_df = pd.DataFrame.from_records(all_records)

        # Store the generated CSV in the session state for download
        st.session_state.csv_export_data = csv_df.to_csv(index=False).encode("utf-8")

    if "csv_export_data" in st.session_state:
        st.success("CSV of EF parameters is ready for download.")
        st.download_button(
            label="Download Parameters CSV",
            data=st.session_state.csv_export_data,
            file_name="ef_parameters_export.csv",
            mime="text/csv",
            on_click=lambda: st.session_state.pop("csv_export_data", None),
        )
